...
```
API 請求 → 檢查快取存在? → 是 → 返回快取資料 → 添加快取命中標頭 → 完成
                       → 否 → 處理請求 → 獲取回應 → 狀態碼=200? → 是 → 儲存至快取 → 返回回應 → 完成
                                                               → 否 → 不快取 → 返回回應 → 完成
```
...
```
開始連線測試 → 檢查環境設定 → 檢查 PHP 擴展 → Socket 層連線測試
              → PHP Redis 擴展測試 → Laravel Redis 測試
              → 生成診斷報告 → 提供解決方案
```
...
```
客戶端請求 → Laravel 路由 → CacheRequestFingerprint 中間件 → 快取命中? → 是 → 返回快取資料 → 完成請求
                                                          → 否 → 控制器處理 → 資料庫查詢
                                                                → 返回資料 → 儲存至快取 → 返回回應 → 完成請求

管理介面 → 快取監控儀表板 → CacheService → Redis 伺服器

Redis 連線測試 → 多層次診斷 → 生成報告
```
